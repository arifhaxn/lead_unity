...
class _TeamListScreenState extends State<TeamListScreen> {
  ...
  String _teamKey(Map<String, dynamic> team) =>
      (team['_id'] ?? team['title'] ?? '').toString();

  List<String> _extractCourseTabs(List<dynamic> teams) {
    final codes = teams
        .map((t) => (t['course'] != null)
            ? t['course']['courseCode'].toString()
            : 'N/A')
        .toSet()
        .toList();
    return codes..sort();
  ...
  bool _hasSubmittedEvaluation(
    ...
    return marks.any((m) =>
        m['supervisorId'].toString() == myId &&
        m['type'].toString() == evaluationType);
  }
  ...
  Widget _buildTeamCard(BuildContext context,
    ...
    String supervisorName = team['assignedSupervisor'] is Map
                    ...
                    children: [
                      ...
                      RichText(
                        text: TextSpan(
                          ...
                          children: [
                            // 🟢 DIFFERENT COLOR FOR DATE (Primary)
                            TextSpan(
                                text: parts['date'],
                                style: TextStyle(
                                    color: theme.colorScheme.primary)),
                            TextSpan(
                                ...
                                style: TextStyle(
                                    color: theme.colorScheme.onSurfaceVariant
                                        .withOpacity(0.4))),
                            // 🟡 DIFFERENT COLOR FOR TIME (Amber/Yellow)
                            TextSpan(
                                text: parts['time'],
                                style:
